...
## Cleaning
```{r}
# Convert am, vs, and cyl columns into factors
mtcars <-
  mtcars %>%
...
summary(fit1)
```
The adjusted R^2 is 0.3385, so this model can explain 33.85% of the data. This is not very high, so we should add more variables to the model so it can explain a larger portion of the data.
